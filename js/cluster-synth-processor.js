class ClusterSynthProcessor extends AudioWorkletProcessor {
    ...
    _configure(config) {
        ...
        this.harmonicMode = config.harmonicMode || false; // NEW: Harmonic mode flag
        ...
        for (let i = 1; i <= this.clusterSize; i++) {
            // NEW: Harmonic Series mode uses integer multiples
            const freq = this.harmonicMode ?
                this.baseFreq * i :
                this.baseFreq + i * this.step;

            this.frequencies[i-1] = freq;
    ...
    process(inputs, outputs) {
        ...
        const waveComp = { sine: 1.0, triangle: 0.9, square: 0.6, sawtooth: 0.7 }[this.waveform];
