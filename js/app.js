...
class App {
    ...
    _randomizePreset() {
        ...
        // If playing, restart with new random settings
    ...
    _savePreset() {
        ...
        const config = {
            ...
            harmonicMode: this.harmonicMode // NEW: Save harmonic mode
        };
    ...
    _loadPreset(name) {
        ...
        if (config) {
            ...
            this.harmonicMode = config.harmonicMode || false; // NEW: Load harmonic mode
            this.harmonicModeCheckbox.checked = this.harmonicMode;
    ...
    _updateStepSizeSliderFromValue(step) {
        const min = 0.00001, max = 50; // Updated to 0.00001
        const val = Math.log(step / min) / Math.log(max / min) * 100;
        this.stepSizeSlider.value = val;
        this.stepValDisplay.textContent = step < 1 ? step.toFixed(5) + ' Hz' : step.toFixed(2) + ' Hz';
    }
    ...
    _updateFilterCutoffSliderFromValue(filterCutoff) {
        const min = 20, max = 22050; // Updated to 22050
    ...
    _updateStepSize(val) {
        ...
        this.stepValDisplay.textContent = this.step < 1 ? this.step.toFixed(5) + ' Hz' : this.step.toFixed(2) + ' Hz';
        this._updateInfo();
        if (this.engine.isPlaying) this._restartCluster();
    }
    ...
    _updateInfo() {
        const mode = this.harmonicMode ? "Harmonic" : "Linear";
        const lowest = this.harmonicMode ?
            this.baseFreq :
            this.baseFreq + this.step;
        const highest = this.harmonicMode ?
            this.baseFreq * this.clusterSize :
            this.baseFreq + this.clusterSize * this.step;
        const spread = this.harmonicMode ?
            (this.clusterSize - 1) * this.baseFreq :
            (this.clusterSize - 1) * this.step;

        this.infoRange.textContent = `${lowest.toFixed(3)} Hz → ${highest.toFixed(3)} Hz (${mode})`;
        this.infoSpread.textContent = spread < 1 ? `${spread.toFixed(5)} Hz` : `${spread.toFixed(2)} Hz`;
    ...
    async _startCluster() {
        ...
        try {
            this.engine.start(
                ...
                this.stereoWidth, this.attackTime, this.releaseTime, this.harmonicMode
            );
        } catch (err) {
